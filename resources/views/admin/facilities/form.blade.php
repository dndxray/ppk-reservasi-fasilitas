        ...
        <form method="POST"
            ...
            <div>
                ...
                >
                    ...
                    @foreach ($tipeOptions as $value => $label)
                        <option
                            value="{{ $value }}"
                            @selected(old('tipe', $facility?->tipe ?? '') === $value)
                        >
                            {{ $label }}
                        </option>
                    @endforeach
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="w-full border-stone-300 rounded-md text-sm"
                >{{ old('deskripsi', $facility?->deskripsi ?? '') }}</textarea>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button
                    type="submit"
                    class="bg-slate-800 text-white text-sm font-semibold rounded-md px-5 py-2 hover:bg-slate-900"
                >
                    {{ $facility?->exists ? 'Simpan Perubahan' : 'Simpan' }}
                </button>
